...
// Full-Portal rows select the product's default portal page (no focus-mode swap here).
const FULL_PORTAL_PAGE = Object.fromEntries(PRODUCTS.filter((p) => p.items?.length).map((p) => [p.id, PORTALS[p.id]?.defaultPage || p.items[0].id]))
...
// Product tile: vibrant blue gradient when subscribed, the muted #0F223D / #618BC2
// treatment when not (matching the original SAT/Archive locked tiles). One glyph drives
// both states, so subscribing/unsubscribing flips the look without swapping assets.
function ProductTile({ glyph, muted, size = 32 }) {
  const uid = useId().replace(/:/g, '')
  if (muted) {
    return (
      <svg width={size} height={size} viewBox="0 0 32 32" fill="none" style={{ display: 'block' }} aria-hidden="true">
        <rect width="32" height="32" rx="8" fill="#0F223D" />
        <path d={glyph} fill="#618BC2" />
      </svg>
    )
  }
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none" style={{ display: 'block' }} aria-hidden="true">
      ...
      <defs>
        <linearGradient id={bg} x1="16" y1="0" x2="16" y2="32" gradientUnits="userSpaceOnUse"><stop stopColor="#0068CB" /><stop offset="1" stopColor="#07101F" /></linearGradient>
        <linearGradient id={bd} x1="16" y1="0" x2="16" y2="32" gradientUnits="userSpaceOnUse"><stop stopColor="#3EB3F0" /><stop offset="1" stopColor="#0068CB" /></linearGradient>
        <linearGradient id={gl} x1="16" y1="8" x2="16" y2="24" gradientUnits="userSpaceOnUse"><stop stopColor="#fff" /><stop offset="1" stopColor="#618BC2" /></linearGradient>
      </defs>
    </svg>
  )
}
...
function ShellNav({
  collapsed, page, openIds, onToggleProduct, onSelectItem, onToggleCollapse, dark, onToggleDark,
                    ...
                    <div style={{ overflow: 'hidden', minHeight: 0 }} aria-hidden={!open}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 2, paddingTop: 8, opacity: open ? 1 : 0, transition: 'opacity 180ms ease' }}>
                        ...
                        <MenuItem collapsed={collapsed} icon={<ArrowUpRight size={16} />} label="Full Portal" labelSize={11} labelWeight={400} color={C.inkDim} fp
                          selected={page === FULL_PORTAL_PAGE[p.id]} onClick={() => onSelectItem(FULL_PORTAL_PAGE[p.id])} />
                      </div>
                    </div>
                  </div>
                </div>
...
function ShellInner() {
  ...
  return (
    <div className="shell-root" style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: C.topbar, overflow: 'hidden', fontFamily: 'var(--vds-font-sans)' }}>
      {/* Top chrome: VIPRE logo strip only — the scope navigator now lives in the nav. */}
